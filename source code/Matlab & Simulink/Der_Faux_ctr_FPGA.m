...
load_audio_sample = false;

if load_audio_sample == true
   [audio, fs] = audioread('Faux_Sample_Guitar_01_in.wav'); % load audio file
   % audio = (audio_stereo(:,1)+audio_stereo(:,2))./2;      % Convert to Mono
    ...
    audio = [audio,audio,audio,audio_max_p*ones(1,3000),audio_max_n*ones(1,3000),2*rem(1:3000,2) - 1]';
end

% --- Simulation parameters ---
ovs = 128;              % Oversampling factor
...
% --- Uncomment those two lines if an optimized Simulink model is being simulated ---
...
f_clk = fs_sim;
shift_reg_size = 8*44000;       % Trade-off between noise and LUT resolution
